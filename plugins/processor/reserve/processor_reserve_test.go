...
func newProcessor(idxs ...int) (*ProcessorReserve, error) {
...
func TestSourceKey(t *testing.T) {
	...
	assert.Equal(t, 5, len(log.Contents))
}
...
func TestSourceKey1(t *testing.T) {
	...
	log := getLog()
	processor.processLog(log)
	assert.Equal(t, 2, len(log.Contents))
...
func TestSourceKey3(t *testing.T) {
	...
	assert.Equal(t, 2, len(log.Contents))
	assert.Equal(t, "test_1", log.Contents[0].Value)
	assert.Equal(t, "test_5", log.Contents[1].Value)
}
